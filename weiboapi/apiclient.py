...
""" Sina Weibo SDK: Naive Version - Just for crawling data.
Support http proxy.
"""
...
class APIClient(object):
    ...
    # **need some manual work: acquire the code from redirect url
    def request_token(self):
        ...
        r = _call_with_proxy(self.auth_url+'access_token',
                method=_HTTP_POST,
                client_id=self.APP_KEY,
    ...
    def call(self, api_method, **kw):
        return self.call_with_proxy(api_method, **kw)

    def call_with_proxy(self, api_method, proxy={}, **kw):  #only for http method "GET"
        url = '%s%s.json' % (self.api_url, api_method)
        return _call_with_proxy(url, access_token=self.access_token, proxy=proxy, **kw)

    def rate_limit(self, proxy={}):
        r = self.call_with_proxy('account/rate_limit_status', proxy=proxy)
        return r
